...
def getInvoice(acc, facility, startP, endP, accName, cycle, wise=False):
    try:
        ...
        if wise:
            ...
            info('-------------------------' + '\n\n\nDownloaded Wise ACtivity Report ' + str(
                datetime.now()) + '\n\n\n-------------------------------------------------')
        else:
            ...
            info('-------------------------' + '\n\n\nDownloaded BNP Invoice ' + str(
                datetime.now()) + '\n\n\n-------------------------------------------------')

            if fileEnd == 'Invoice[Multi].xlsx':
                rename(os.path.join(accsdir, '00 - Historical Invoices', fileEnd),
                       os.path.join(accsdir, '00 - Historical Invoices', accName + "-" + facility + "-" + fileEnd))
...
if __name__ == '__main__':
    ...
    for index in invoiceAccs.index:
        ...
        info('-------------------------' + f'\n\n\nGetting Invoice for {accName}-{fac} ' + str(
            datetime.now()) + '\n\n\n-------------------------------------------------')
    ...
    info(('Completion Time - %s seconds' % (
                time() - start_time)) + '\n\n\n-------------------------------------------------')
